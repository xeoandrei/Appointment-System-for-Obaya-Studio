	...
	if($stmt = mysqli_prepare($link, $sql))
	{
		...
		if(mysqli_execute($stmt))
		{
			...
			if(mysqli_num_rows($result) != 1)
			{
				...
				{
					$_SESSION['add-women-error'] = 'You are allowed to only upload images with only jpg, png, jpeg and gif formats';
				}
				else
				{
					...
					{
						$filename = $image;
						$_SESSION['add-women-error'] = 'Image Already exists';
					}
					else
					{
						//insert
							...
							if(mysqli_stmt_execute($stmt))
							{
                        		...
                        		if($stmt = mysqli_prepare($link, $sql))
								{
                            		...
                            		if(mysqli_stmt_execute($stmt))
                            		{
										...
										$_SESSION['add-women-success'] = 'A New Women\'s Service is Successfully Created!';
										header("location: manage_women_services.php");
										exit();
                            		}
                            		else
                            		{
                                		$_SESSION['add-women-error'] = "Error on inserting logs";
                            		}	
								}
								else
								{
									$_SESSION['add-women-error'] = "Error on before inserting logs";
								}
							}
							else
							{
								$_SESSION['add-women-error'] = "Error on insert service statement";
							}
						}
						else
						{
							$_SESSION['add-women-error'] = "Error on before inserting service";
						}
					}
				}		
			}
			...
			{
				$_SESSION['add-women-error'] = "Service is already in use";
			}
		}
		else
		{
			$_SESSION['add-women-error'] = "Error on select statment";
		}
	}
	else
	{
		$_SESSION['add-women-error'] = "Error on prepare statment";
	}
}
?>
<html>
...
<div class="container-fluid row">	
		...
		<div class="card mx-auto shadow p-3 mb-5 bg-body rounded col-lg-6 col-md-8">
			<div class="mt-5">
				...
				<?php
					if(isset($_SESSION['add-women-error']))
					{
						echo'<div class="alert alert-danger text-center">
                                ' . $_SESSION['add-women-error'] . 
                            '</div>';
                            unset($_SESSION['add-women-error']);
					}
				?>
				<div class="card-body">
