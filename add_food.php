	...
	if($stmt = mysqli_prepare($link, $sql))
	{
		...
		if(mysqli_execute($stmt))
		{
			...
			if(mysqli_num_rows($result) != 1)
			{
				...
				{
					$_SESSION['add-food-error'] = 'You are allowed to only upload images with only jpg, png, jpeg and gif formats';
				}
				else
				{
					...
					{
						$filename = $image;
						$_SESSION['add-food-error'] = 'Image Already exists';
					}
					else
					{
						//insert
							...
							if(mysqli_stmt_execute($stmt))
							{
                        		...
                        		if($stmt = mysqli_prepare($link, $sql))
								{
                            		...
                            		if(mysqli_stmt_execute($stmt))
                            		{
										...
										$_SESSION['add-food-success'] = 'A New Food Item is Successfully Created!';
										header("location: manage_food.php");
										exit();
                            		}
                            		else
                            		{
                                		$_SESSION['add-food-error'] = "Error on inserting logs";
                            		}	
								}
								else
								{
									$_SESSION['add-food-error'] = "Error on before inserting logs";
								}
							}
							else
							{
								$_SESSION['add-food-error'] = "Error on insert food item statement";
							}
						}
						else
						{
							$_SESSION['add-food-error'] = "Error on before inserting food item";
						}
					}
				}		
			}
			...
			{
				$_SESSION['add-food-error'] = "Food item is already in use";
			}
		}
		else
		{
			$_SESSION['add-food-error'] = "Error on select statment";
		}
	}
	else
	{
		$_SESSION['add-food-error'] = "Error on prepare statment";
	}
}
?>
<html>
...
<div class="container-fluid row">	
		...
		<div class="card mx-auto shadow p-3 mb-5 bg-body rounded col-lg-6 col-md-8">
			<div class="mt-5">
				...
				<?php
					if(isset($_SESSION['add-food-error']))
					{
						echo'<div class="alert alert-danger text-center">
                                ' . $_SESSION['add-food-error'] . 
                            '</div>';
                            unset($_SESSION['add-food-error']);
					}
				?>
				<div class="card-body">
